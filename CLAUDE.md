...
Env in `.env.local`: `PINTEREST_CLIENT_ID`, `PINTEREST_CLIENT_SECRET`, `PINTEREST_REDIRECT_URI`, `SESSION_SECRET`, `OPENROUTER_API_KEY`, `OPENROUTER_MODEL` (default `anthropic/claude-sonnet-4.6`), `TURSO_DATABASE_URL` (optional in dev — falls back to `file:./data/designr.db`), `TURSO_AUTH_TOKEN` (required when `TURSO_DATABASE_URL` is remote).

## Deployment

Two artifacts, deployed separately:

- **`site/`** — static marketing landing (`index.html`, `privacy/`). Lives at Dreamhost webroot `~/designr.quest/` (apex `designr.quest`) via `ssh designr` (host alias in `~/.ssh/config`). Push with `npm run deploy:site` (rsync with `--delete`, excludes `.dh-diag` symlink and `favicon.*` placeholders that Dreamhost owns). Marketing CTA points at `https://app.designr.quest/api/auth/pinterest`.
- **Next.js app** — the actual swipe app at `app.designr.quest`. Deployed via Vercel (GitHub-connected, pushes to `main` auto-deploy). All env vars must be set in Vercel dashboard — `.env.local` is dev-only.
...
- **libSQL / Turso** (`@libsql/client`) — tiny KV keyed on Pinterest user id for persisting swipe sessions + analysis + chat across devices. Prod uses a Turso-hosted libSQL DB (`TURSO_DATABASE_URL` + `TURSO_AUTH_TOKEN`); dev falls back to `file:./data/designr.db` (git-ignored). Schema auto-creates on first query. See `src/lib/db.ts`.
...
src/
  ...
  lib/
    ...
    db.ts                               libSQL/Turso KV: list/get/upsert/delete sessions
site/
...
- **Library backend** — sessions auto-save to libSQL/Turso on finish; analysis and chat debounce-save on change. `/library` lists past sessions per Pinterest user id.
...
- The `data/` directory is git-ignored; don't commit the local libSQL file.
